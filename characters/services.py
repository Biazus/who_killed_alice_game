...
from .models import CharacterModifier, CharacterModifierAttribute
...
class CharacterService(object):
    ...
    def heal(self, amount):
        if not amount:
            ...
            self.character.current_health = min(
                self.character.current_health + int(amount),
    ...
    def attach_random_modifiers_to_character(self, num_modifiers: int = None):
        ...
        for modifier in chosen_modifiers:
            cm = CharacterModifier.objects.create(character=self.character, modifier=modifier)
    ...
    def get_character_attribute_values(self) -> dict:
        char_mod_attr = CharacterModifierAttribute.objects.filter(character_modifier__character=self.character)
        char_mod_attr_values = char_mod_attr.values('attribute', 'attribute__code', 'attribute__name').annotate(total=Sum('final_value'))
        res = {a["attribute__code"]: a["total"] + BASE_ATTRIBUTE for a in char_mod_attr_values}
        return res