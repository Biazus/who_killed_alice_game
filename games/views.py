...
class SceneActionSelectView(APIView):
    ...
    def _get_or_create_character_progress(
        ...
        if not character_progress:
            ...
            first_act = Act.objects.order_by("id").first()
            if not first_act:
                raise ValueError(
            ...
            first_scene_of_act = first_act.scenes.order_by("order").first()
    ...
    def post(self, request, *args, **kwargs):
        ...
        if scene_action.action_type == "P" and scene_action.action:
            action_service = ActionService(character)
            action_result = action_service.resolve_action_min_gate(
                scene_action.action.code
            )
            ...
            if action_result["success"]:
                ...
                # Lógica de falha crítica TODO melhorar
                if roll_value >= 99 and scene_action.on_hard_fail:
                    next_scene = scene_action.on_hard_fail
                    message = (
                        f"{base_message} ... e as coisas pioraram! (Falha Crítica)"
                    )
                else:
        ...
        if not next_scene:
            ...
            final_message = (
                f"{message} O caminho se encerra aqui. Progresso finalizado."
            )
