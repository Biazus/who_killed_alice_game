...
class SceneActionSelectView(APIView):
    ...
    def _get_or_create_character_progress(
        self, character_id: int
    ) -> CharacterProgressOnAct:
        """
        ...
        character_progress = CharacterProgressOnAct.objects.filter(
            character=character, finished=False
        ).first()

        if not character_progress:
            # Se não houver progresso ativo, inicia um novo
            first_act = Act.objects.order_by(
                "id"
            ).first()  # Ou por um campo 'order' se você tiver
            if not first_act:
                raise ValueError(
                    "Nenhum Ato configurado no sistema para iniciar o jogo."
                )

            # Pega a primeira cena do primeiro ato, ordenada pelo campo 'order'
            first_scene_of_act = first_act.scenes.order_by(
                "order"
            ).first()  # <-- Ajustado para usar related_name 'scenes' e 'order'
            if not first_scene_of_act:
                raise ValueError(
                    f"O Ato '{first_act.title}' não possui cenas configuradas."
                )
    ...
    def post(self, request, *args, **kwargs):
        ...
        threshold = (
            scene_action.action.difficulty if scene_action.action else 10
        )  # Exemplo: dificuldade base
        ...
        if scene_action.action_type == "P" and scene_action.action:
            if roll_result >= threshold:
                ...
                message = (
                    f"Você rolou {roll_result} e obteve sucesso! Aventura continua..."
                )
            elif scene_action.on_hard_fail and roll_result <= (
                threshold / 2
                ...
                message = (
                    f"Você rolou {roll_result} e falhou. A história toma outro rumo..."
                )
        else:
        ...
        if not next_scene:
            ...
            return Response(
                {
                    "detail": f"{message} O caminho se encerra aqui. Progresso finalizado.",
                    "finished": True,
                },
        ...
        response_data["game_message"] = (
            message  # Adiciona a mensagem do jogo à resposta
        )
