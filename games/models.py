...
class Scene(models.Model):
    ...
    description = models.TextField(blank=True)
    act = models.ForeignKey(Act, on_delete=models.CASCADE, related_name="scenes") # <-- Adicionado related_name
...
class SceneAction(models.Model):
    ...
    scene = models.ForeignKey(Scene, on_delete=models.CASCADE, related_name="scene_actions") # <-- Adicionado related_name
    description = models.TextField(blank=True)
    ...
    def __str__(self): # <-- Adicionado método __str__
        return f"[{self.scene.title if self.scene else 'N/A'}] {self.description[:50]}..."

class CharacterProgressOnAct(models.Model):
    character = models.ForeignKey(Character, on_delete=models.CASCADE, related_name="progresses") # <-- Adicionado related_name
    act = models.ForeignKey(Act, on_delete=models.CASCADE, related_name="progresses_on_act") # <-- Adicionado related_name e Act
    current_scene = models.ForeignKey("games.Scene", on_delete=models.CASCADE, related_name="current_progresses")
