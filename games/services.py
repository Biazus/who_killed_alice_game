...
class ActionService:
    ...
    def resolve_action_min_gate(self, action_code: str) -> dict:
        action = Action.objects.prefetch_related('requirements__attribute').get(code=action_code)
        ...
        total_difficulty = action.difficulty + sum(r["req"].difficulty_delta for r in attr_values)
